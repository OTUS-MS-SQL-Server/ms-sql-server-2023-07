...
; with cte as (select value from string_split(space(3999), ' '))
insert resumable_table (name)
select concat('name ', row_number() over(order by 1/0)) from cte as t1, cte as t2

create nonclustered index nix_resumable_table on resumable_table(name)
*/
--

alter index nix_resumable_table on resumable_table rebuild with (online = on, resumable = on)

--проверка доступа (в новом окне)
select top 10 * from resumable_table 
...
---
--обычный rebuild
alter index nix_resumable_table on resumable_table rebuild

--проверка доступа (в новом окне)
select top 10 * from resumable_table --доступа нет